            ...
            <div className="bg-emerald-900 rounded-3xl p-8 relative overflow-hidden shadow-xl shadow-emerald-900/10">
              ...
              <div className="relative z-20 max-w-xl text-white">
                <h2 className="text-3xl font-bold mb-2">
                  Tabik Pun, {currentUser.name.split(" ")[0]}! 👋
                </h2>
                <p className="text-emerald-100 mb-6 text-lg">
                  Siap menjelajahi pesona Sai Bumi Ruwa Jurai hari ini?
                </p>
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {recommendedDestinations.map((dest) => (
                    <div
                      key={dest.id}
                      className="bg-white rounded-3xl overflow-hidden border border-slate-100 hover:shadow-xl transition-all"
                    >
                      <img
                        src={dest.image}
              ...
              <div className="space-y-8">
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-100">
                  ...
                  {recentBookings.map((booking) => (
                    <div
                      key={booking.id}
                      className="flex gap-4 items-center p-3 bg-slate-50 rounded-2xl"
                    >
                      <img
                        src={booking.image}
                      ...
                      <span
                        className={`text-[9px] px-2 py-1 rounded-md border font-bold ${getStatusColor(booking.status)}`}
                      >
                        {booking.status}
                      </span>
                    </div>
...
function StatCard({
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center gap-5">
      <div
        className={`w-14 h-14 ${bg} rounded-2xl flex items-center justify-center`}
      >
