...
import { 
  ...
  Play,
  View,
  LayoutGrid, // Icon tambahan untuk tombol Grid
  Map // Icon tambahan untuk tombol Map
} from 'lucide-react';
...
type ViewMode = 'grid' | 'map'; // Tipe data baru untuk melacak mode tampilan
...
interface Destination {
  ...
  isPopular?: boolean;
  has360?: boolean;
  // Tambahan koordinat persentase agar posisi pin responsif dan pas di dalam container peta
  mapX: string; 
  mapY: string;
}
...
export default function Eksplorasi() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>('grid'); // State pengatur mode view aktif
  const [hoveredPin, setHoveredPin] = useState<string | null>(null); // State mendeteksi pin peta yang disorot
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 w-full p-6 lg:p-10 pb-20 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {/* PENUKAR TAMPILAN (TOGGLE VIEW MODE: GRID / MAP) */}
      ...
      {/* KONDISIONAL AREA KONTEN (TAMPILAN GRID ATAU TAMPILAN PETA) */}
      {filteredDestinations.length > 0 ? (
        viewMode === 'grid' ? (
          /* ================= TAMPILAN 1: GRID MODE ================= */
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7 pt-2 animate-in fade-in duration-300">
          ...
          /* ================= TAMPILAN 2: MAP MODE ================= */
          <div className="w-full h-[600px] bg-slate-200 border border-slate-300/60 rounded-[2.5rem] relative overflow-hidden shadow-inner flex items-center justify-center animate-in fade-in duration-300">
            {/* Latar Belakang Tekstur Peta Abstrak */}
            <div className="absolute inset-0 opacity-15 bg-[radial-gradient(#059669_1.5px,transparent_1.5px)] [background-size:24px_24px] pointer-events-none" />
            <div className="absolute top-1/2 left-1/3 w-96 h-96 bg-emerald-300/20 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute bottom-1/3 right-1/4 w-[500px] h-[500px] bg-blue-300/20 rounded-full blur-3xl pointer-events-none" />

            {/* Label Informasi Peta */}
            <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-md px-5 py-3 rounded-2xl shadow-md border border-slate-100 z-10">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Peta Wilayah</p>
              <h4 className="text-base font-extrabold text-slate-800">Provinsi Lampung</h4>
            </div>

            {/* ITERASI PIN LOKASI DESTINASI */}
            {filteredDestinations.map((dest) => (
              <div
                key={dest.id}
                className="absolute z-10 transition-transform duration-300"
                style={{ top: dest.mapY, left: dest.mapX }}
              ...
              >
                ...
                <div className="relative flex items-center justify-center cursor-pointer group">
                  {/* Efek Gelombang Pulsa pada Pin Aktif */}
                  <div className="absolute w-8 h-8 bg-emerald-500/30 rounded-full animate-ping group-hover:bg-emerald-600/40" />
                  
                  {/* Kepala Pin Utama */}
                  <div className="w-10 h-10 bg-emerald-600 text-white rounded-full flex items-center justify-center shadow-lg group-hover:bg-slate-900 border-2 border-white transition-colors duration-300 relative z-10">
                    <MapPin className="w-5 h-5" />
                  </div>

                  {/* POP-UP PREVIEW MINIMALIS (Muncul saat Pin disentuh mouse) */}
                  {(hoveredPin === dest.id) && (
                    <div className="absolute bottom-12 w-52 bg-white rounded-2xl p-2.5 shadow-[0_15px_30px_rgba(0,0,0,0.12)] border border-slate-100 z-30 animate-in fade-in slide-in-from-bottom-2 duration-200 pointer-events-none">
                      <div className="w-full h-24 rounded-xl overflow-hidden mb-2">
                        <img src={dest.image} alt={dest.name} className="w-full h-full object-cover" />
                      </div>
                      <h5 className="font-bold text-slate-800 text-sm line-clamp-1">{dest.name}</h5>
                      <p className="text-[11px] font-medium text-slate-400 flex items-center gap-1 mt-0.5">
                        <MapPin className="w-3 h-3 text-emerald-500" /> {dest.location}
                      </p>
                      <div className="flex justify-between items-center mt-2 pt-1.5 border-t border-slate-100">
                        <span className="text-[10px] font-bold text-slate-400">Mulai dari</span>
                        <span className="text-xs font-extrabold text-emerald-600">{dest.price}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Petunjuk Interaksi */}
            <div className="absolute bottom-6 bg-slate-900/80 backdrop-blur text-white text-xs font-bold px-4 py-2 rounded-full pointer-events-none shadow">
              💡 Arahkan kursor ke PIN untuk melihat pratinjau lokasi
            </div>
          </div>
        )
      ) : (
        ...
        <div className="w-full flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-slate-500 max-w-md">
            Maaf, tidak ada destinasi dengan filter atau kata kunci tersebut.
          </p>
        </div>
      )}
